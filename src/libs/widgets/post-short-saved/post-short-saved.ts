...
@Component({
  ...
  templateUrl: './post-short-saved.html',
  styleUrl: './post-short-saved.css',
})
export class PostShortSavedComponent implements OnChanges {
  @Input() data: any;
  ...
  savePost(post: any) {
    ...
    setTimeout(() => {
      this.store.dispatch(
        actions.prepareFetchUserSavedPost({ email: userLoginedObject.email })
      );
    }, 500);
  }
